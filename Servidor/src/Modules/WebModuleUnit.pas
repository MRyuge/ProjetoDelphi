...
begin
  ...
  Writeln('Requisição recebida: ' + ARequest);
  Writeln('Segmentos: ' + ASegments.Text);
  Writeln('Método chamado: ' + ADSMethodName);

  // Verificar se é uma requisição DELETE para ExcluirPessoa
  if (ADSMethodName = 'ExcluirPessoa') and (ASegments.Count > 0) then
  begin
    Writeln('Interceptando chamada para ExcluirPessoa com parâmetro: ' +
      ASegments[0]);
    // Adicione a lógica desejada aqui
  end;

  // Permitir que o dispatcher continue processando
end;

procedure TWebModule1.DSServerClass1GetClass(DSServerClass: TDSServerClass;
  var PersistentClass: TPersistentClass);
begin
  PersistentClass := ServerMethodsUnit.TDSServerModule1;
end;
...
begin
  if SameText(TagString, 'urlpath') then
    ...
    ReplaceText := ServerMethodsUnit.TDSServerModule1.ClassName
