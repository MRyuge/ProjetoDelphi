...
function TDSServerModule1.BuscarEndereco(IdEndereco: Int64): TJSONObject;
var
  DAO: TEnderecoDAO;
  Endereco: TEndereco;
  JSONResult: TJSONObject;
begin
  ...
  try
    ...
    Endereco := DAO.GetById(IdEndereco);
    if Assigned(Endereco) then
    begin
      // Preenche o objeto JSON com os dados do endereço
      JSONResult.AddPair('IdEndereco', TJSONNumber.Create(Endereco.IdEndereco))
        .AddPair('IdPessoa', TJSONNumber.Create(Endereco.IdPessoa))
        .AddPair('DsCep', Endereco.DsCep);
    end
    else
...
begin
  DAO := TEnderecoDAO.Create;
  try
    // Verifica o retorno do método Delete
  ...
  try
    ...
    try
      ...
      begin
        ...
        raise Exception.CreateFmt
          ('Endereço de integração com ID %d não encontrado.', [IdEndereco]);
    finally
