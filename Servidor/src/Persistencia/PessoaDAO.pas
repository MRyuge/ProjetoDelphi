...
uses
  ...
  Data.DB, Pessoa, System.Generics.Collections, DataModuleDatabase, System.JSON;
...
type
  ...
  public
    ...
    function PessoaExiste(IdPessoa: Int64): Boolean;
  end;
...
function TPessoaDAO.PessoaExiste(IdPessoa: Int64): Boolean;
var
  Query: TFDQuery;
begin
  Query := TFDQuery.Create(nil);
  try
    Query.Connection := dmDatabase.FDConnection; // Conexão ao banco
    Query.SQL.Text := 'SELECT 1 FROM pessoa WHERE idpessoa = :IdPessoa';
    Query.ParamByName('IdPessoa').AsLargeInt := IdPessoa; // Define o parâmetro
    Query.Open;

    Result := not Query.IsEmpty; // Verifica se o resultado não está vazio
  finally
    Query.Free; // Libera o recurso
  end;
end;
...
begin
  ...
  try
    Query.Connection := dmDatabase.FDConnection;
    // Certifique-se de que dmDatabase está configurado corretamente
    ValuesSQL := '';
    ...
    begin
      ...
      // Adiciona os valores do JSON à SQL
      if ValuesSQL <> '' then
        ValuesSQL := ValuesSQL + ', ';
