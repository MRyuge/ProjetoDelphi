...
begin
  try
    Response := THttpClientUtils.Get(FBaseUrl + '/AtualizarIntegracaoViaCep');
    try
...
function TEnderecoService.BuscarEnderecoIntegracao(IdEndereco: Int64)
  : TJSONObject;
var
  Response: TJSONValue;
  JSONResponse: TJSONObject;
begin
  ...
  try
    // Realiza a requisição GET para buscar a integração de endereço
    Response := THttpClientUtils.Get(FBaseUrl + '/BuscarEnderecoIntegracao/' +
      IdEndereco.ToString);
    try
      // Verifica se a resposta foi retornada e é um JSON válido
      if Assigned(Response) and (Response is TJSONObject) then
      begin
        JSONResponse := TJSONObject(Response);
        Result := TJSONObject(JSONResponse.Clone); // Retorna uma cópia do JSON
      end
      else
        raise Exception.Create('Resposta da API não é um objeto JSON válido.');
    finally
    ...
    on E: Exception do
      raise Exception.Create('Erro ao buscar a integração do endereço: ' +
        E.Message);
  end;
end;
